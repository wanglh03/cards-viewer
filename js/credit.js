...
function mapCreditCard(bankKey, bankInfo, entry, mycardIssuer = {}) {
  const cardMeta = entry.card || entry;
  if (cardMeta.type !== "Credit") return null;

  const sharedBillingDay = firstDefined(mycardIssuer.billing_day);
  const sharedDueDay = firstDefined(mycardIssuer.due_day);
  const sharedLimitValue = firstDefined(mycardIssuer.limit);
  const limitValue = sharedLimitValue != null ? sharedLimitValue : cardMeta.limit;
  const sharedLimit = sharedLimitValue != null;
  ...
  return {
    ...
    supplementary,
    billingDay:
      sharedBillingDay != null
        ? sharedBillingDay
        : firstDefined(cardMeta.billing_day),
    dueDay: sharedDueDay != null ? sharedDueDay : firstDefined(cardMeta.due_day),
