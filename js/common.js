(() => {
  ...
  function discoverIssuers(generatedInfo = {}) {
    return Object.keys(generatedInfo).filter((key) => {
      const info = generatedInfo[key];
      return (
        info &&
        typeof info === "object" &&
        info.bank &&
  ...
  function mergeIssuerData(info, mycardData, options = {}) {
    ...
    return {
      ...
      issuer:
        mycardData?.issuer &&
        typeof mycardData.issuer === "object" &&
  ...
  async function loadCardsFromAssets(mapEntry, options = {}) {
    ...
    for (const item of issuers) {
      ...
      if (!data || !data.bank || !Array.isArray(data.cards)) {
        if (warn) {
          console.warn(`Skipped issuer data: ${bankKey}`);
        }
        continue;
      }
  ...
  const brandLabel = brand.label || "卡面图鉴";
