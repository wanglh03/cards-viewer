(() => {
  ...
  async function discoverAssetFolders() {
    const preloaded = getPreloadedSiteData();
    if (Array.isArray(preloaded?.issuerKeys)) {
      return preloaded.issuerKeys;
    }
  ...
  function mergeIssuerData(info, mycardData) {
    ...
    const mycards = new Map(
      (mycardData?.cards || [])
        ...
        .map((card) => [String(card.name), card]),
    );
    return {
      bank: info.bank,
  ...
  async function loadCardsFromAssets(mapEntry, options = {}) {
    ...
    const preloaded = getPreloadedSiteData();
    const [generatedInfo, generatedMycards] = await Promise.all([
    ...
    for (const item of issuers) {
      ...
      const assetUrl = assetPath("assets", "issuers", bankKey, "data.json");
      const data =
        preloaded?.issuers?.[bankKey] ||
        mergeIssuerData(generatedInfo?.[bankKey], generatedMycards?.[bankKey]) ||
        (await fetchJsonSafe(assetUrl, { warn }));
      if (!data || !data.bank || !Array.isArray(data.cards)) {
        if (warn) {
          console.warn(`Skipped issuer data: ${assetUrl}`);
        }
        continue;
      }
