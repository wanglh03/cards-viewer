...
def write_site_data(output_dir: Path, data: dict[str, object]) -> None:
    generated_dir = output_dir / "js" / "generated"
    issuer_info = data.get("issuerInfo", {})
    mycards = data.get("mycards", {})
    if not isinstance(issuer_info, dict) or not isinstance(mycards, dict):
        raise ValueError("generated issuer data must be objects")
    ...
    write_text(generated_dir / "issuer-info.json", f"{payload(issuer_info)}\n")
    write_text(generated_dir / "mycards.json", f"{payload(mycards)}\n")

    site_payload = {
        key: value
        for key, value in data.items()
        if key not in {"issuerInfo", "mycards"}
    }
    site_payload["issuerInfoUrl"] = "js/generated/issuer-info.json"
    site_payload["mycardsUrl"] = "js/generated/mycards.json"
    write_text(
        generated_dir / "site-data.js",
        f"window.__CARDS_VIEWER_DATA__ = {payload(site_payload)};\n",
    )
